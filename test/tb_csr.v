...
module tb_csr;
    ...
    initial begin
        ...
        rd_en=0; wr_en=0; trap_en=0; mret_en=0; sret_en=0; addr=0; wdata=0; trap_cause=0; trap_epc=0; trap_val=0;
        rst_n=0; @(negedge clk); @(negedge clk); rst_n=1;

        // citiri read-only
        rd_chk(1, `CSR_MISA, `MISA_RV32IMA);
        ...
        if (trap_vec_o !== 32'h0000_1240) begin errors=errors+1; $display("  FAIL: trap_vec_o=%h", trap_vec_o); end
        ...
        if (ret_pc_o !== 32'h0000_0080) begin errors=errors+1; $display("  FAIL: ret_pc_o=%h", ret_pc_o); end
